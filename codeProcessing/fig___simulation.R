...
df.pixList <- data.frame(pixLbl = c('A','B','C'),
                         grd_id = c(
                           ...
                           which.min(abs(df.sum$TCI-quantile(df.sum$TCI, 0.95))),
                           which.min(abs(df.sum$TCI-quantile(df.sum$TCI, 0.5))),
                           which.min(abs(df.sum$TCI-quantile(df.sum$TCI, 0.1)))
                         )) %>% 
...
map.class <- ggplot(df.grd, aes(y = y, x = x)) +
  geom_raster(aes(fill = factor(original_id))) + 
...
fig.width <- 16; fig.height <- 12;  fig.fmt <- 'pdf'
