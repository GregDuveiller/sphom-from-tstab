...
- pur_avg : the mean purity value
- pur_std : the standard deviation of the purity
...
ggplot(df.sum.long %>% 
         filter(pur_avg > 0.5)) +
  geom_point(aes(x = pur_avg, y = value_var,
...
ggplot(df.sum.long %>% 
         filter(pur_avg > 0.5)) +
  geom_point(aes(x = pur_std, y = value_var,
                 colour = factor(original_id)),
             shape = 3) +
  facet_grid(var_type ~ ., scales = 'free_y') +
  scale_x_log10() +
...
ggplot(df.sum %>% filter(pur_avg > 0.5)) +
  geom_point(aes(x = pur_avg, y = var_sig/var_tot,
                 colour = factor(original_id)),
             shape = 3) +
  scale_x_log10() +
  scale_colour_manual(values = c('1' = col.1, '0' = col.2), guide = 'none')
...
```{r}
ggplot(df.sum %>% filter(pur_avg > 0.5)) +
  geom_point(aes(x = pur_std, y = var_sig/var_tot,
                 colour = factor(original_id)),
             shape = 3) +
  scale_x_log10() +
  scale_colour_manual(values = c('1' = col.1, '0' = col.2), guide = 'none')
...
```{r}
ggplot(df.sum %>% filter(pur_avg > 0.5)) +
  geom_point(aes(x = pur_avg, y = var_sig/var_res,
                 colour = factor(original_id)),
             shape = 3) +
  scale_x_log10() +
  scale_colour_manual(values = c('1' = col.1, '0' = col.2), guide = 'none')
...
```{r}
ggplot(df.sum %>% filter(pur_avg > 0.5)) +
  geom_point(aes(x = pur_std, y = var_sig/var_res,
                 colour = factor(original_id)),
             shape = 3) +
  scale_x_log10() +
  scale_colour_manual(values = c('1' = col.1, '0' = col.2), guide = 'none')
